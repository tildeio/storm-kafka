...
public class PartitionManager {
  ...
  public PartitionManager(
    DynamicPartitionConnections connections, String topologyInstanceId, ZkState state, Map stormConf,
  ...
  private void fill() {
    //LOG.info("Fetching from Kafka: " + _consumer.host() + ":" + _partition.partition + " from offset " +
    // _emittedToOffset);
    ...
    ByteBufferMessageSet msgs = _consumer.fetch(
      new FetchRequestBuilder().addFetch(
        ...
        _emittedToOffset,
        _spoutConfig.fetchSizeBytes).build()).messageSet(
      _spoutConfig.topic,
      _partition.partition);
...
/*        if(numMessages>0) {
    ...
    for (MessageAndOffset msg : msgs) {
      ...
      _emittedToOffset = msg.nextOffset();
        ...
        /*if(numMessages>0) {
          LOG.info("Added " + numMessages + " messages from Kafka: " + _consumer.host() + ":" + _partition.partition
          + " to internal buffers");
        }*/
  }
