...
public class StaticCoordinator implements PartitionCoordinator {
  ...
  public StaticCoordinator(
    DynamicPartitionConnections connections, Map stormConf, SpoutConfig config, ZkState state, int taskIndex,
    ...
    for (int i = taskIndex; i < partitions.size(); i += totalTasks) {
      ...
      _managers
        .put(myPartition, new PartitionManager(connections, topologyInstanceId, state, stormConf, config, myPartition));
